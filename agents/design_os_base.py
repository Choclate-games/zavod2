...
Чтобы «только онлайн» не означало «прогон умирает от одной осечки», вызов
повторяется: сеть рвётся, CLI-агент упирается в лимит, процесс убивают по
таймауту — всё это обычно лечится второй попыткой. Когда попытки кончились,
прогон не падает, а ПРИОСТАНАВЛИВАЕТСЯ (`RunPaused`): сделанное лежит в сессии,
и `continue <run_id>` продолжает со следующего шага, не переспрашивая модель о
том, что она уже ответила.
