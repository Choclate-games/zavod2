...
# Верхняя граница выбора. Раньше стояла на 12 «чтобы набор был узким», и это
# была неверно поставленная задача. Опасны не лишние документы вообще, а лишние
# ЖАНРОВЫЕ: документ про орду в игре про доставку — инструкция построить чужую
# игру. Документ про производительность, звук или рэгдолл чужого жанра принести
# не может, зато его отсутствие стоит дорого: в тактическом шутере не оказалось
# ни рэгдолла, ни бюджета кадра именно из-за этого потолка.
MAX_SELECTIONS = 18
...
SYSTEM_PROMPT = (
    ...
    f"2. Не больше {MAX_SELECTIONS} документов. Опасен не лишний документ вообще, а лишний "
    "ЖАНРОВЫЙ: документ про орду в игре о доставке — прямая инструкция построить чужую игру. "
    "Документ про производительность, звук или физику тел чужой жанр принести не может.\n"
    "2b. Набор обязан быть ШИРОКИМ, а не глубоким в одной папке. Пять документов вокруг "
    "главной механики и ноль про то, на чём игра работает, — типичная и дорогая ошибка. "
    "Закрой каждую ось хотя бы одним документом:\n"
    f"{knowledge.axes_summary()}\n"
    "Если по какой-то оси в базе нет подходящего документа — так и напиши в summary.\n"
...
class KnowledgeCuratorAgent:
    ...
    def _normalize(cls, plan: Optional[KnowledgePlan], ctx: GenerationContext) -> KnowledgePlan:
        ...
        # Оси, которые куратор не закрыл, добираются здесь. Это не «на всякий
        # случай»: без документа про производительность игра греет телефон,
        # без документа про тела в кадре в шутере не оказывается рэгдолла.
        cls._cover_axes(plan, forbidden)
    ...
    @staticmethod
    def _cover_axes(plan: KnowledgePlan, forbidden: dict) -> None:
        """Добирает по одному документу на каждую незакрытую ось."""
        for addition in knowledge.fill_axes(plan.paths(), forbidden):
            if len(plan.selections) >= MAX_SELECTIONS:
                log_agent(
                    "KnowledgeCurator",
                    f"[warn]Ось «{addition['axis']}» осталась не закрыта: набор упёрся "
                    f"в потолок {MAX_SELECTIONS} документов[/warn]",
                )
                break
            plan.selections.append(KnowledgeSelection(
                path=addition["path"], role="supporting",
                reason=f"Ось «{addition['axis']}» не была закрыта выбором куратора. {addition['why']}",
            ))
            log_agent(
                "KnowledgeCurator",
                f"Добран документ по оси «{addition['axis']}»: {addition['path']}",
            )
