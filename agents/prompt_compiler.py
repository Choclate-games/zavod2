...
class PromptCompilerAgent:
    ...
    def _knowledge_block(cls, concept) -> str:
        ...
        mandatory = "\n".join(row(rel, with_checklist=True) for rel in knowledge.MANDATORY_TOPICS)
        ...
        parts = [
            ...
            "ошибку и проверяется взглядом на запущенную игру. Всё, что ты "
            "оставляешь незакрытым, объясняется строкой в `DEVLOG.md`.",
