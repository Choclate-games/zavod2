...
def contract_block(raw_prompt: str) -> str:
    ...
    if anchors:
        parts.append(
            "Названо в промпте и обязано сохраниться в направлении проекта дословно:\n"
            + "\n".join(f"- {a.label}" for a in anchors)
        )
    parts.append(
        "Свобода у тебя в мире, твисте, подаче и деталях, о которых промпт молчит, — но не "
        "в подмене жанра. Направление, где заказанного жанра нет, — это невыполненный заказ, "
        "а не оригинальность."
    )
    parts.append(
...
def enforce(direction, raw_prompt: str):
    ...
    Досталось от времён, когда директор писал три направления сразу: перенести
    выбор было дешевле, чем переспрашивать. Направление теперь одно, и функция
    почти всегда только сообщает о потере — чинит её переспрос в
    `ProjectDirectorAgent._insist_on_the_order`. Ветку переноса держим для
    прогонов, восстановленных из старых снимков: там варианты ещё лежат."""
    anchors = anchors_for(raw_prompt)
    ...
    if rescue is None or len(kept_by(rescue, anchors)) <= len(kept_by(selected, anchors)):
        return direction, (
            f"Направление не удержало заказ: потеряно — {', '.join(a.label for a in lost)}."
        )
