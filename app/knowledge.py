...
CORE_TOPICS: List[str] = [
    ...
    # Схем управления всегда две, и переключает их площадка (bridge.device.type),
    # а не догадки браузера. Без этого документа агент собирает одну гибридную
    # раскладку: тач-слой висит поверх мыши на ПК, а на телефоне половина
    # действий остаётся на клавишах — управление не подходит ни тем, ни другим.
...
# Уроки фабрики собираются из отчётов приёмки уже выпущенных игр
# (`app/gate_stats.py`) и потому обязательны: это единственный документ базы,
# который знает, на чём спотыкались предыдущие проекты, а не как правильно
# вообще. Файла может не быть на свежей установке — `resolve` пропустит
# несуществующий путь сам.
LESSONS_TOPIC = "FACTORY_LESSONS.md"

MANDATORY_TOPICS: List[str] = list(CORE_TOPICS) + [LESSONS_TOPIC]
