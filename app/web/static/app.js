...
const state = {
  ...
  elapsed: 0,
  streamBubble: null,
  streamBubbles: [],   // потоковые пузыри текущего ответа — их заменит Markdown-версия
};
...
function pushChatEvent(event) {
  ...
  if (kind === "user") {
    ...
    const body = event.text ? `<div class="md" style="margin-top:6px">${renderMarkdown(event.text)}</div>` : "";
    addBubble("result",
