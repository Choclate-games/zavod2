...
#
# Замок лежит рядом с остальным состоянием деплоя, а не в /tmp с общим именем.
# Общее имя в /tmp — это отказ на ровном месте: файл, созданный однажды другим
# пользователем (руками, из-под root), навсегда роняет деплой с «Permission
# denied» ещё до первой полезной команды. Запасной путь на случай, если
# каталог состояния недоступен, различает пользователей по uid.
LOCK="${LOCK:-$STATE_DIR/deploy.lock}"
if ! ( : > "$LOCK" ) 2>/dev/null; then
    LOCK="/tmp/zavod2-deploy.$(id -u).lock"
fi
exec 9>"$LOCK"
if flock -n 9; then
