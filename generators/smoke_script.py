"""Скрипт дымового запуска, уезжающий в каждый пакет игры.

`check-spec.mjs` читает исходники: он ловит недописанное, но не умеет открыть
игру. Пакет мог пройти всю статическую приёмку и не запуститься — упасть на
сборке из-за необъявленного импорта CSS, бросить исключение в первом кадре,
показать чёрный экран или разъехаться на телефоне. Ни одну из этих поломок
чтение исходников не видит, а игрок видит все.

Скрипт собирает проект, поднимает сервер, открывает игру в системном Chromium
по протоколу DevTools, считает кадры и вызовы отрисовки, жмёт кнопки и трогает
экран пальцем — на десктопе и на телефоне. Зависимостей нет: сборка через npm,
сервер из node:http, сокет — встроенный в Node 22.
"""

SMOKE_MJS = r'''#!/usr/bin/env node
/**
...
function build() {
  if (FLAGS.has('--skip-build')) {
    ...
    pass('S1', 'Сборка проходит', 'пропущена по --skip-build')
    return true
  }
...
const PROBE = `(() => {
  ...
  window.requestAnimationFrame = (cb) => raf((t) => { s.frames++; return cb(t) });
  const patch = (proto) => {
    if (!proto) return;
    for (const name of ['drawArrays', 'drawElements', 'drawArraysInstanced', 'drawElementsInstanced']) {
      const original = proto[name];
      if (typeof original !== 'function') continue;
      proto[name] = function (...args) { s.draws++; return original.apply(this, args) };
    }
  };
...
async function run() {
  ...
  try {
    ...
    await wait(1500)
    const before = await evaluate(cdp, desktop, 'window.__smoke && { ...window.__smoke }')
    ...
    await wait(2000)
    const after = await evaluate(cdp, desktop, 'window.__smoke && { ...window.__smoke }')
