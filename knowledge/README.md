...
| `threejs/skinned_character_models.md` | Готовые FBX из `assets/`: нормализация и перепривязка, свои оси поверх чужого скелета, рэгдолл из костей (разбитых по суставам), ретаргет мокапа по направлениям и его четыре ловушки — включая ту, из-за которой удар уходит вбок, — плюс размер дописанных аксессуаров, плюс запекание мокапа в ПРОЦЕДУРНУЮ анимацию (ряды Фурье по фазе шага, ключевые позы дельтой, реквизит по двум кистям и перевод его из мировых координат в локальные, 5.6 МБ FBX → 22 КБ кода) |
...
| `threejs/fps_controller_and_shooting.md` | Знаки осей (откуда «управление инвертировано»), прыжок с койот-таймом и буфером, опора рейкастом по BVH, захват мыши из обработчика нажатия, событие против удержания для полуавтомата, таблица оружия и смена ствола, вьюмодель с руками и ловушка `lookAt` (+Z), пружина отдачи, вспышка/трассер/декаль/гильза/взрыв, прицел как индикатор разброса, фиксация сцены при съёмке кадров |
| `threejs/shooter_enemy_ai_and_combat.md` | Hitscan, TTK, enemy FSM, attack tokens, cover, hit feedback, анимация врага двух уровней — коробки и скиненная модель с мокапом, реакция на попадание, кровь тремя слоями, оружие по двум кистям, калибровка наводки отдельно от разворота тела, смерть рэгдоллом, срок жизни тела, horde |
| `threejs/melee_combat_and_ragdoll.md` | Кадры вместо секунд, связки и буфер ввода, hit-stop, парирование, токены атаки, рэгдолл на Rapier и рэгдолл на верле без физдвижка (связки «не ближе, чем», трение через прошлую позицию, сон, возврат позы направлениями) |
| `threejs/juice_and_vfx_pool.md` | Пул частиц на 1 draw call, пер-партикловые гравитация/сопротивление/рост, направленный конус выброса, затухание через `setColorAt`, ловушка кэша `boundingSphere` у мировых инстансов, шейк камеры |
