...
> Тот же контроллер уместен и там, где движением управляет не физика, а
> детерминированная логика: в файтинге логика считает желаемое смещение, а
> Rapier отвечает, куда можно (канаты, угол ринга, второй боец). Границу
> «что отдать физике, а что нет» разбирает
> `knowledge/threejs/fighting_game_core.md` §8.
