# FINAL AI DEVELOPER PROMPT: КУРИНЫЙ ПОБЕГ 3D: СТЕЛС НА ФЕРМЕ 🎮⚡
...
- **Game Title**: Куриный Побег 3D: Стелс на Ферме
- **Project Slug**: `chicken-stealth-farm-escape-3d`
...
- **Session Model**: Быстрые напряженные сессии по 1.5–3 минуты на уровень с нарастающей комплексностью патрулей и мета-прогрессией скинов.
...
- **State Management**: Zustand + TinyEventEmitter
...
### Изометрическое перемещение курицы (CORE)
- **Category**: movement
...
- **Player Input**: WASD / Стрелки на клавиатуре или аналоговый экранный джойстик на мобильных устройствах.
- **Hit & Sensory Feedback**: Частицы пыли из-под лапок, характерный звук топота и покачивание гребешка курицы.
- **Technical Complexity**: Low

### Маскировка под коробку (Box Hide) (CORE)
- **Category**: stealth
...
- **Player Input**: Кнопка 'Пробел' на ПК или крупная плавающая сенсорная кнопка с иконкой коробки справа на экране смартфона.
- **Hit & Sensory Feedback**: Звук глухого хлопка картона, частицы облачка пыли, анимация опускания коробки с глазками-щелками.
- **Technical Complexity**: Medium

### Динамический конус зрения собак (Vision Cone System) (CORE)
- **Category**: ai
...
- **Player Input**: Визуальное считывание безопасных зон и траекторий движения охранников для планирования тайминга перебежек.
- **Hit & Sensory Feedback**: Конус плавно пульсирует полупрозрачным мягким зеленым/желтым цветом в спокойном режиме и вспыхивает агрессивным красным светом при обнаружении.
- **Technical Complexity**: Medium

### Сбор зерен и механизм открытия ворот (CORE)
...
- **Player Input**: Перемещение курицы через триггер-зоны зерен.
- **Hit & Sensory Feedback**: Приятный звонкий звук подбора монетки, вылетающие золотые звездочки и заполнение круговой шкалы ключа в HUD.
- **Technical Complexity**: Low

### ИИ тревоги и погони (Alert & Pursuit AI) (CORE)
- **Category**: ai
...
- **Player Input**: Убегание за угол или своевременная остановка под коробкой вне прямого контакта.
- **Hit & Sensory Feedback**: Над головой собаки появляется анимированный знак '!' со звуком сирены Metal Gear, пес издает грозный лай.
- **Technical Complexity**: Medium
...
- **Мгновенное спасение (Revive) (`revive_checkpoint`)**: Возрождает курицу на безопасном расстоянии от собаки с временной 3-секундной неуязвимостью и сохраненным прогрессом собранных зерен. (Trigger: Экран поимки собакой (Game Over Screen), Limit: 1 раз за уровень)
- **Удвоение урожая зерен (`double_grain_reward`)**: Удваивает количество собранных на уровне золотых зерен для ускоренной разблокировки скинов. (Trigger: Экран победы (Victory Screen), Limit: После каждого успешного уровня)
- **Скин 'Курица-Ниндзя' в аренду (`unlock_ninja_skin`)**: Предоставляет доступ к уникальному скину с бесшумным бегом на 3 уровня. (Trigger: Магазин скинов / Экран выбора уровня, Limit: Без ограничений)
...
- Persistent storage key: `"player_progress_level"` — one key, one JSON object.
...
Проект живёт в песочнице `workspace/chicken-stealth-farm-escape-3d/`, и вся работа за её пределы
...
- **Style**: Stylized Low-Poly Toon Farm
- **Camera Perspective**: Изометрическая проекция (Isometric 3D Camera, Pitch 50°, Yaw 45°) (FOV: 45°, Pitch: 50°)
- **Environment**: Яркая, солнечная мультяшная ферма с деревянными заборчиками, красными амбарами, тюками соломы, грядками моркови и мощеными дорожками.
- **Lighting**: Теплый направленный свет солнца (DirectionalLight с мягкими тенями), мягкий небесный AmbientLight и яркие светящиеся полупрозрачные конусы прожекторов зрения (Spotlight Mesh) с градиентным затуханием.
...
- **Deliverable**: Игрок может бегать по полигону и прятаться под коробку по кнопке.
...
- **Deliverable**: Полноценный стелс-геймплей с уклонением от патрулей собак.
...
- **Deliverable**: Завершенный цикл прохождения уровней от старта до выхода.
...
- **Deliverable**: Релизный билд, готовый к публикации на Yandex Games и Playgama.
...
- [ ] Курица плавно управляется с клавиатуры (WASD) и сенсорного джойстика с плавной физикой.
- [ ] Кнопка 'Коробка' мгновенно маскирует курицу; неподвижная курица в коробке не триггерит тревогу патрульных собак.
- [ ] Конусы зрения собак визуализированы красивыми градиентными полупрозрачными мешами и корректно блокируются заборами.
- [ ] При входе незамаскированной курицы в конус включается тревога (!), собака с лаем бросается в погоню.
- [ ] Все золотые зерна собираются, открывая запертые ворота уровня со спецэффектом.
- [ ] Интегрирован Playgama Bridge SDK: работают Rewarded Ads за ревайв, Interstitials между уровнями и сохранение прогресса.
- [ ] Стабильные 60 FPS в мобильных и десктопных браузерах при весе бандла менее 3.5 МБ.
