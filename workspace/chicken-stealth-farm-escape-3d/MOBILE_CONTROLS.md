# Mobile Controls & Ergonomics: Куриный Побег 3D: Стелс на Ферме

Профиль управления: **default** (определён по жанру «3D Стелс-головоломка / Аркадный Экшен»).

## 1. Orientation & Layout
- **Target Orientation**: **LANDSCAPE**
- **Safe Area Insets**: `Адаптивные отступы env(safe-area-inset-*) для вырезов экранов iPhone/Android, фиксация соотношения сторон изометрической камеры.`
