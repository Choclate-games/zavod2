# Mobile Controls & Ergonomics: CYBER SLICE: BULLET PROTOCOL (Кибер Срез: Протокол Времени)

Профиль управления: **default** (определён по жанру «Экшен-слэшер / Аркада»).

## 1. Orientation & Layout
- **Target Orientation**: **LANDSCAPE**
- **Safe Area Insets**: `Автоматический отступ критических UI элементов от челок и закруглений экрана через CSS env(safe-area-inset-*).`
