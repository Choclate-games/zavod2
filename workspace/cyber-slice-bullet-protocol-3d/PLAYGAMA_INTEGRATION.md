# Playgama Bridge Integration: CYBER SLICE: BULLET PROTOCOL (Кибер Срез: Протокол Времени)

## 1. SDK Overview
- **SDK**: `@playgama/bridge 2.x`
- **Supported Portals**: yandex, vk, crazygames, gamepix, telegram, standalone_web, android_capacitor

## 2. Initialization Flow
- Инициализация bridge.initialize() при старте загрузчика Vite.
- Определение платформы окружения и типа управления (touch/desktop mouse).
- Загрузка сохраненного профиля игрока через bridge.storage.get(['cyber_profile', 'settings']).
- Синхронизация локализации bridge.platform.language (RU/EN).
- Отправка события bridge.game.ready() после загрузки 3D-ассетов сцены.
...
- `cyber_slice_high_score`
- `boss_rush_time_attack`
- `max_combo_streak`
