# FINAL AI DEVELOPER PROMPT: ЛЕСНОЙ РЕЙС: ДОСТАВКА НА ЛЕСОПИЛКУ 3D 🎮⚡
...
- **Game Title**: Лесной Рейс: Доставка на Лесопилку 3D
- **Project Slug**: `rustic-cargo-truck-3d`
- **Genre**: Физический 3D-симулятор вождения (Аркадная доставка груза по бездорожью)
- **Target Platform**: Playgama Bridge (Яндекс Игры / VK Play / Web & Mobile)
...
- **Session Model**: Быстрые динамичные сессии по 2–4 минуты на один заезд с постоянным накоплением прогресса и апгрейдами.
...
### Двухпедальное управление тягой и торможением (CORE)
- **Category**: driving
- **Description**: Игрок управляет только подачей газа (ускорение вперед) и тормозом (замедление / задний ход / балансировка в воздухе).
- **Player Input**: Нажатие клавиш W/S, стрелок вверх/вниз на ПК или сенсорных экранных педалей на смартфонах.
- **Hit & Sensory Feedback**: Звук нарастающих оборотов двигателя, дым из выхлопной трубы, наклон кузова грузовика при торможении.
- **Technical Complexity**: Medium

### Физическая симуляция груза в кузове (CORE)
- **Category**: physics
...
- **Player Input**: Косвенное влияние через скорость и ускорение грузовика.
- **Hit & Sensory Feedback**: Стук дерева о дерево, глухие удары о борта, визуальное смещение и вываливание предметов через край.
- **Technical Complexity**: High

### Процедурная генерация ухабистого маршрута (CORE)
- **Category**: generation
- **Description**: Генерация 3D-сплайна дороги с наложением шумов Перлина/Симплекс для создания кочек, оврагов, канав и мостов.
- **Player Input**: Игрок визуально оценивает рельеф впереди и заранее сбрасывает или набирает скорость.
- **Hit & Sensory Feedback**: Тряска камеры, срабатывание рессор подвески, пыль из-под колес при наезде на неровности.
- **Technical Complexity**: High

### Индикатор целостности и подсчет доставленного груза (CORE)
- **Category**: scoring
- **Description**: Отслеживание количества оставшихся в кузове объектов в реальном времени и расчет финальной награды на финише.
- **Player Input**: Игрок следит за шкалой на HUD и выбирает между риском ради скорости и осторожностью.
- **Hit & Sensory Feedback**: Цветовая смена индикатора (зеленый -> желтый -> красный), тревожный звуковой сигнал при выпадении предмета.
...
- **Удвоить награду за рейс (`double_reward`)**: Умножает заработанные монеты за доставленный груз на x2 (Trigger: Экран завершения рейса на лесопилке, Limit: После каждого успешного заезда)
- **Магнит груза (Возврат) (`cargo_magnet_revive`)**: Возвращает все выпавшие брёвна обратно в кузов и чинит целостность до 100% (Trigger: Критическая потеря груза во время заезда, Limit: 1 раз за заезд при потере >50% груза)
- **Тест-драйв супер-подвески (`free_super_tuning`)**: Дает максимальный уровень подвески и шин на один следующий рейс (Trigger: Экран гаража перед стартом, Limit: 1 раз в 10 минут)
...
Проект живёт в песочнице `workspace/rustic-cargo-truck-3d/`, и вся работа за её пределы
...
- **Style**: Cozy Rustic Low-Poly Stylized 3D
- **Camera Perspective**: Динамическая преследующая камера сбоку-сзади (Chase Cam 3/4) с плавным демпфированием (FOV: 52°, Pitch: 22°)
- **Environment**: Живописная русская глубинка: деревянные избы с резными наличниками, золотые сосны, березовые рощи, речной брод с деревянным мостом и дымящая лесопилка вдали.
- **Lighting**: Мягкий теплый свет 'золотого часа' (DirectionalLight под низким углом с теплыми оранжевыми тенями) + рассеянный небесный свет HemisphericalLight.
...
- **Deliverable**: Управляемый грузовик, способный перевозить подвижные брёвна в кузове по тестовой плоскости с трамплинами.
...
- **Deliverable**: Полноценная процедурная трасса от деревни до лесопилки с ухабами, кочками и мостами.
...
- **Deliverable**: Красивая и завершенная играбельная сессия доставки груза с полным визуальным и звуковым фидбеком.
...
- **Deliverable**: Полноценный релизный билд, готовый к публикации на Яндекс Играх и мобильных платформах.
...
- [ ] Грузовик плавно управляется с клавиатуры (W/S/Стрелки) и сенсорных педалей на смартфонах
- [ ] Процедурная ухабистая трасса генерируется с кочками, спусками, мостами и корректными коллизиями
- [ ] Физические брёвна/ящики реалистично подпрыгивают в кузове и выпадают при экстремальной езде
- [ ] Спидометр и индикатор целостности груза на HUD точно отражают текущую скорость и оставшийся товар
- [ ] Заезд завершается подсчетом доставленного груза у лесопилки с начислением монет
- [ ] Реализована система улучшений в гараже (двигатель, подвеска, шины, борта)
- [ ] Интегрирован Playgama Bridge (сохранения, лидерборды, показ рекламы)
- [ ] Стабильные 60 FPS на ПК и мобильных устройствах
