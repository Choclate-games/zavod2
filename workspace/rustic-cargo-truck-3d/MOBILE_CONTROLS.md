# Mobile Controls & Ergonomics: Лесной Рейс: Доставка на Лесопилку 3D

Профиль управления: **default** (определён по жанру «Физический 3D-симулятор вождения»).

## 1. Orientation & Layout
- **Target Orientation**: **LANDSCAPE**
- **Safe Area Insets**: `Применение CSS env(safe-area-inset-left) и env(safe-area-inset-right) для предотвращения наложения кнопок на вырезы камер смартфонов.`
