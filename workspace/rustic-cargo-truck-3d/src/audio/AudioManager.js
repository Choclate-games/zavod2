export class AudioManager {
    ...
    setPlatformMuted(muted) {
        ...
        if (this.master && this.context)
            this.master.gain.setTargetAtTime(muted || this.muted ? 0.0001 : .14, this.context.currentTime, .12);
    }
    unlock() {
        if (!this.context) {
    ...
    updateEngine(speed, throttle, muted) {
        this.muted = muted;
    ...
    playTone(frequency, duration, volume) {
        if (!this.context || !this.master || this.muted || this.platformMuted || this.context.state !== 'running')
            return;
