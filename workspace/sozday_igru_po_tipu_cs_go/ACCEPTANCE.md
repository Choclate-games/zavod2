...
- [ ] **O3** · Игра осталась тем, что просили: стрельба как главное действие игрока.
- [ ] **O4** · Игру узнают как то, о чём просили (cs go): жанр и главное действие те же, мир и твист — свои.
...
- [ ] **B9** · Экраны проекта существуют и переключаются по одному: MainMenu, WeaponShop, DuelHUD, RoundEndOverlay, MatchVictoryDefeat, LeaderboardModal, SettingsModal.
...
- [ ] **C6** · Награда за rewarded выдаётся только по `state === 'rewarded'` (Удвоение награды за матч), слушатель снимается через `off()`, повторный клик не платит дважды.
...
- [ ] **C9** · Возможность, которой на площадке нет, не нарисована вовсе — не серой кнопкой и не ошибкой по нажатию. Проверяется подменой флага (global_elo_ladder (Рейтинг дуэлянтов)).
