...
- `docs/ref/knowledge/threejs/tower_defense_core.md` — Tower Defense на Three.js: маршрут, приоритет целей, снаряды, волны — Сетка размещения и постройки — `grid_building.md` и `base_building.md`. Здесь — то, что делает TD именно TD: как враги идут, как башни выбирают цель…
