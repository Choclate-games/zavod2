# Mobile Controls & Ergonomics: Воксельный Измельчитель ASMR 3D

Профиль управления: **default** (определён по жанру «Казуальный симулятор / Кликер»).

## 1. Orientation & Layout
- **Target Orientation**: **PORTRAIT**
- **Safe Area Insets**: `Адаптивные CSS переменные env(safe-area-inset-top) и env(safe-area-inset-bottom) для корректного отображения на экранах с вырезами (iPhone/Android).`
